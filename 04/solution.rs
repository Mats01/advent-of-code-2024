...
fn check_direction(grid: &Vec<Vec<String>>, start: [i32; 2], direction: [i32; 2]) -> bool {
    ...
    for i in 1..4 {
        if grid[(start[0] + (direction[0] * i)) as usize][(start[1] + (direction[1] * i)) as usize]
            != need_to_find[i as usize]
        {
            return false;
        }
    }
    ...
    for i in 0..grid.len() as i32 {
        for j in 0..grid[0].len() as i32 {
            if grid[i as usize][j as usize] == "X" {
                for dx in -1..2 {
                    for dy in -1..2 {
                        if check_direction(&grid, [i, j], [dx, dy]) {
                            total_finds += 1;
                        }
                    }
                }
